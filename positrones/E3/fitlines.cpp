...
Int_t  fitlines(){  
  ...
  TF1 *fit = new TF1("positron fit", Positron, 0.5, 1500., 6);
  fit->SetParameters(1.10, 6.51e-02, -4.07, 6.8e-05, -2.58, 1.23e-3);
  //fit->SetParameters(1.11, 6.49e-02, -4.10, 6.75e-05, -2.61, 1.20e-3);
  ...
  TColor *color = gROOT->GetColor(kRed);
  color->SetRGB(0.9, 0.68, 0.11);

  TLatex tsou;
  tsou.SetTextSize(0.06);
  tsou.SetTextColor(kRed);
  tsou.DrawLatex(100,11, "Source e^{+}");

  TColor *color2 = gROOT->GetColor(kCyan);
  color2->SetRGB(0.78, 0.23, 0.04);

  tsou.SetTextSize(0.05);
  tsou.SetTextColor(kCyan);
  tsou.DrawLatex(2.5,5, "Diffusion e^{+}");
  //c1->SaveAs("positron_v2.pdf");
