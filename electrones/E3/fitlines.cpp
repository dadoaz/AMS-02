  ...
  //-----------------------------------------------------------
	...
	Double_t C_d       = par[1];
 	Double_t gamma_d   = par[2];
  	...
  	Double_t E3        = par[9];
  	Double_t E_d       = par[10];  
  	Double_t E_s       = par[11];//This parameter add (3) or not (0) a factor E^3 to the flux function
  	

  	diffusion = C_d*TMath::Power(((E + phi)/E_d),gamma_d);
  	source    = C_s*TMath::Power(((E + phi)/E_s),gamma_s)*TMath::Exp(-(E + phi)*inv_E_s);
